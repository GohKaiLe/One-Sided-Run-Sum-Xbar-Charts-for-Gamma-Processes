        ...
        for S3 = S2:5 
            for S4 = S3:10
                for c = 0.3:0.1:2 % Specify the interval for K
                    xx = ARLFindLowerSteady(n, c, 0, S1, S2, S3, S4, a_shape, b_scale) ; 
                    if xx < ARLi
                        a = c ; 
                    else
                        b = c ; 
                ...
                for i = 1:100 % Start Bisection method for finding K
                    p = (a + b) / 2 ;
                    f = ARLFindLowerSteady(n, p, 0, S1, S2, S3, S4, a_shape, b_scale) ; 
                ...
                if abs(ARL0 - ARLi) < 1 
                    ARL1 = ARLFindLowerSteady(n, K, delta, S1, S2, S3, S4, a_shape, b_scale) ; 
                    SDRL1 = SDRLFindLowerSteady(n, K, delta, S1, S2, S3, S4, a_shape, b_scale) ; 
                    out = [out; S1, S2, S3, S4, K, ARL0, ARL1] ;
                    if ARL1 < ARL1min
                        ARL1min = ARL1 ;
                        ARL0min = ARL0 ; 
                        SDRL1min = SDRL1 ; 
